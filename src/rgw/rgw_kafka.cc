...
connection_id_t::connection_id_t(
    ...
    bool _ssl,
    bool _verify_ssl,
    boost::optional<const std::string&> _ssl_certificate_location,
    boost::optional<const std::string&> _ssl_key_location,
    boost::optional<const std::string&> _ssl_key_password)
  ...
  if (_ssl_certificate_location.has_value()) {
    ssl_certificate_location = _ssl_certificate_location.get();
  }
  if (_ssl_key_location.has_value()) {
    ssl_key_location = _ssl_key_location.get();
  }
...
bool operator==(const connection_id_t& lhs, const connection_id_t& rhs) {
  return lhs.broker == rhs.broker && lhs.user == rhs.user &&
         ...
         lhs.ssl_certificate_location == rhs.ssl_certificate_location &&
         lhs.ssl_key_location == rhs.ssl_key_location &&
...
struct connection_id_hasher {
  std::size_t operator()(const connection_id_t& k) const {
    ...
    boost::hash_combine(h, k.ssl_certificate_location);
    boost::hash_combine(h, k.ssl_key_location);
...
struct connection_t {
  ...
  const boost::optional<std::string> ssl_certificate_location;
  const boost::optional<std::string> ssl_key_location;
  ...
  connection_t(CephContext* _cct, const std::string& _broker, bool _use_ssl, bool _verify_ssl,
          ...
          boost::optional<const std::string&> _ssl_key_location,
          boost::optional<const std::string&> _ssl_key_password) :
      cct(_cct), broker(_broker), use_ssl(_use_ssl), verify_ssl(_verify_ssl),
      ca_location(_ca_location), user(_user), password(_password), mechanism(_mechanism),
      ssl_certificate_location(_ssl_certificate_location),
      ssl_key_location(_ssl_key_location),
      ssl_key_password(_ssl_key_password) {}
...
bool new_producer(connection_t* conn) {
  ...
  if (conn->use_ssl) {
    ...
    if (conn->ssl_certificate_location) {
      if (rd_kafka_conf_set(conf.get(), "ssl.certificate.location", conn->ssl_certificate_location->c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) goto conf_error;
      ldout(conn->cct, 20) << "Kafka connect: successfully configured client certificate location (mTLS)" << dendl;
    }
    if (conn->ssl_key_location) {
      if (rd_kafka_conf_set(conf.get(), "ssl.key.location", conn->ssl_key_location->c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) goto conf_error;
      ldout(conn->cct, 20) << "Kafka connect: successfully configured client key location (mTLS)" << dendl;
    }
    if (conn->ssl_key_password) {
      ...
      ldout(conn->cct, 20) << "Kafka connect: successfully configured client key password (mTLS)" << dendl;
    }
...
public:
  ...
  bool connect(connection_id_t& conn_id,
               ...
               boost::optional<const std::string&> ssl_certificate_location,
               boost::optional<const std::string&> ssl_key_location,
    ...
    connection_id_t tmp_id(broker_list, user, password, ca_location, mechanism,
                           use_ssl, verify_ssl, ssl_certificate_location,
